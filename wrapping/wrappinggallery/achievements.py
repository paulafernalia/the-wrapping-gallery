...
def count_difficulty_carries(general_ratings, difficulty, threshold):
    ...
    return (
        general_ratings.annotate(rounded_difficulty=Round("difficulty"))
        .filter(rounded_difficulty=difficulty)
        .count()
        >= threshold
    )
...
def count_dh_carries(user_done_carries, threshold):
    return (
        user_done_carries.filter(carry__longtitle__icontains="Double Hammock").count()
        >= threshold
    )
...
def count_pirates(user_done_carries, threshold):
    return (
        user_done_carries.filter(carry__name__icontains="pirate").count() >= threshold
    )
...
def count_toddler_prisoner(general_ratings, threshold):
    ...
    # Filter ratings based on the difficulty
    return (
        general_ratings.filter(
            leaners__gte=4, bigkids__gte=4, legstraighteners__gte=4
        ).count()
        >= threshold
    )
...
ACHIEVEMENT_FUNCTIONS = {
    ...
    "one_back_carry": (
        count_position_carries,
        "done_carries",
        {"threshold": 1, "position": "back"},
    ),
    "one_tandem": (
        count_position_carries,
        "done_carries",
        {"threshold": 1, "position": "tandem"},
    ),
    "one_front": (
        count_position_carries,
        "done_carries",
        {"threshold": 1, "position": "front"},
    ),
    ...
    "one_beginner_plus": (
        count_difficulty_carries,
        "general_ratings",
        {"difficulty": 2, "threshold": 1},
    ),
    "one_intermediate": (
        count_difficulty_carries,
        "general_ratings",
        {"difficulty": 3, "threshold": 1},
    ),
    "one_advanced": (
        count_difficulty_carries,
        "general_ratings",
        {"difficulty": 4, "threshold": 1},
    ),
    "one_guru": (
        count_difficulty_carries,
        "general_ratings",
        {"difficulty": 5, "threshold": 1},
    ),
