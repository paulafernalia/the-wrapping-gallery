...
def update_rating(carry):
    # Calculate average ratings from UserRating for the given carry
    newborns = UserRating.objects.filter(carry=carry).exclude(newborns=0).aggregate(newborns=Avg('newborns'))
    bigkids = UserRating.objects.filter(carry=carry).exclude(bigkids=0).aggregate(bigkids=Avg('bigkids'))
    leaners = UserRating.objects.filter(carry=carry).exclude(leaners=0).aggregate(leaners=Avg('leaners'))
    legstraighteners = UserRating.objects.filter(carry=carry).exclude(legstraighteners=0).aggregate(legstraighteners=Avg('legstraighteners'))
    feeding = UserRating.objects.filter(carry=carry).exclude(feeding=0).aggregate(feeding=Avg('feeding'))
    quickups = UserRating.objects.filter(carry=carry).exclude(quickups=0).aggregate(quickups=Avg('quickups'))
    fancy = UserRating.objects.filter(carry=carry).exclude(fancy=0).aggregate(fancy=Avg('fancy'))
    difficulty = UserRating.objects.filter(carry=carry).exclude(difficulty=0).aggregate(difficulty=Avg('difficulty'))
    ...
    averages["votes"] = UserRating.objects.filter(carry=carry).count()

    # Prepare values for the Rating model, replacing None with 0
    rating_values = {key: (value if value is not None else 0) for key, value in averages.items()}

    # Update the Rating model entry for the carry
    Rating.objects.update_or_create(
        carry=carry,
        defaults={
            'newborns': rating_values['newborns'],
            'bigkids': rating_values['bigkids'],
            'leaners': rating_values['leaners'],
            'legstraighteners': rating_values['legstraighteners'],
            'feeding': rating_values['feeding'],
            'quickups': rating_values['quickups'],
            'fancy': rating_values['fancy'],
            'difficulty': rating_values['difficulty'],
            'votes': rating_values['votes'],
        }
    )
