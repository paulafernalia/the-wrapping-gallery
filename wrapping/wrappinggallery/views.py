...
from . import utils
from .forms import CustomLoginForm, CustomUserCreationForm, UserUpdateForm
from .models import (
    Achievement,
    Carry,
    DoneCarry,
...
def achievements(request):
    ...
    user_achievements = UserAchievement.objects.filter(user=request.user).values_list(
...
def index(request):
    ...
    context["layers_values"] = [
        "Varies" if x == -1 else x for x in context["layers_values"]
    ]
...
def collection(request):
    ...
    positions = (
        Carry.objects.values_list("position", flat=True).distinct().order_by("position")
    )
    ...
    annotated_carries = filtered_carries.values("position", "size").annotate(
    ...
    for position in positions:
        ...
        for size in sizes:
            ...
            user_done = user_done_carries.filter(
                carry__position=position, carry__size=size
            )
...
def filter_carries(request):
    ...
    finishes = {key: value for value, key in Carry._meta.get_field("finish").choices}
    ...
    if (
        ...
        and request.GET.get("sortBy") is not None
    ):
...
def get_done_carries(request):
    if request.user.is_authenticated:
        carries = DoneCarry.objects.filter(user=request.user).values_list(
